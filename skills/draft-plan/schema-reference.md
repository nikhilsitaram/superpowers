...
**Handoff notes:** When a task consumes output from a prior phase, the orchestrate lead appends a handoff section to the target task file at the source phase's wrap-up (post-review), so notes reflect the shipped interface:
...
The orchestrate lead writes to `{PLAN_DIR}/phase-{letter}/{target_task_id_lower}.md`, appending the handoff section after the H1 header and before the existing content.
...
│   ├── Writes cross-phase handoff notes: {PLAN_DIR}/phase-{next_letter}/{target_task_id_lower}.md
│   │     (post-review, so notes reflect shipped interface)
│   └── Ships: per-phase PR
│
├── After all phases:
...
- The dispatcher receives `{PLAN_DIR}` so it can call `validate-plan`. Cross-phase handoff notes are written by the orchestrate lead at phase wrap-up (post-review), not by the dispatcher per task.
...
- **Dispatcher receives {PLAN_DIR}** — enables `validate-plan` calls. Cross-phase handoff notes are written by the orchestrate lead at phase wrap-up.
