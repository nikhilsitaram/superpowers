...
2. If `REVIEWER_NEEDED` is `"false"`: record a skip in `reviews.json` (`"verdict":"skip","reason":"reviewer_needed: false"`) and jump to step 6. If `REVIEWER_NEEDED` is `"true"`: dispatch a `claude-caliper:task-reviewer` teammate with dynamic context from `./task-reviewer-prompt.md`, using the task's branch-specific diff range (task worktree `BASE..HEAD`, not the phase-wide range)
...
8. **Record task-review:** Write a passing record to `reviews.json` (in the plan directory): `jq '. += [{"type":"task-review","scope":"{TASK_ID}","verdict":"pass","remaining":0}]' "$PLAN_DIR/reviews.json" > "$PLAN_DIR/reviews.json.tmp" && mv "$PLAN_DIR/reviews.json.tmp" "$PLAN_DIR/reviews.json"`. Create the file (`echo '[]'`) if it doesn't exist. When `reviewer_needed: false`, the skip is recorded at step 2 before reaching this point. All tasks reaching step 8 have passed review.
