...
- [ ] A1: Add workflow and phase depends_on validation to validate-plan — *validate-plan --schema validates workflow enum (ship|review-only|plan-only), phase-level depends_on arrays (valid letters only, backward references), and detects circular phase dependencies; all new schema tests pass*
